...
class SourceKyzqfqgg:
    __domain__ = 'https://athlete.kyzqfqgg.com/'
    ...
    def parse(self):
        ...
        r = requests.post(url, data=raw_body.to_dict()).json()
        self.data = r['result']['data']
        ...
        for row in self.data:
            # 取得 issue
            issues.append(row['periods'])

            tmp_code = []
            for code in row['ball']:
                tmp_code.append(code['num'].zfill(2))
            codes.append(','.join(tmp_code))
